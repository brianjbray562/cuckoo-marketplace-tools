...
function AppInner() {
  ...
  const generateBackendKeywords = useCallback(async () => {
    ...
    try {
      ...
      // #6 — Diff against frontend copy + backfill reclaimed bytes with competitor/alt-language/long-tail pool
      if (parsed.keywords && (bkCurrentTitle.trim() || bkCurrentBullets.trim())) {
        try {
          const optimized = optimizeBackendKeywords(parsed.keywords, bkCurrentTitle.trim(), bkCurrentBullets.trim(), dbProduct || { sku: bkModelNumber }, 499);
          parsed.keywords = optimized.keywords;
          parsed.byte_count = optimized.byte_count;
          parsed._bkOptimization = { removed: optimized.removed, added: optimized.added.map(a => a.text), bytesReclaimed: optimized.bytesReclaimed };
        } catch (e) {
          // Fallback to raw LLM output if optimizer errors
        }
      }
      setBkResults(parsed);
    } catch (e) {
  ...
  const generateFullListing = useCallback(async () => {
    ...
    try {
      ...
      // #6 — Diff BK against title+bullets, backfill with competitor/alt-language pool
      if (keywords?.keywords) {
        try {
          const optimized = optimizeBackendKeywords(keywords.keywords, amazonTitle, bulletText, product, 499);
          keywords.keywords = optimized.keywords;
          keywords.byte_count = optimized.byte_count;
          keywords._bkOptimization = { removed: optimized.removed, added: optimized.added.map(a => a.text), bytesReclaimed: optimized.bytesReclaimed };
        } catch (e) {
          // Keep raw LLM output on error
        }
      }
    ...
    for (let i = 0; i < sortedModels.length; i++) {
      ...
      try {
        ...
        if (keywords?.keywords) {
          ...
          // #6 — Diff against title+bullets, backfill with pool
          try {
            const optimized = optimizeBackendKeywords(keywords.keywords, amazonTitle, bulletText, { sku, ...product }, 499);
            keywords.keywords = optimized.keywords;
            keywords.byte_count = optimized.byte_count;
            keywords._bkOptimization = { removed: optimized.removed, added: optimized.added.map(a => a.text), bytesReclaimed: optimized.bytesReclaimed };
          } catch (e) { /* keep raw LLM output on error */ }
        }
        // Validation pass
        const validation = validateListingOutput(titles, bullets, keywords, product);
